...
class NavalWargamingLexem:
    ...
    def __str__(self):
        return f"{self.tag}({self.value}) {self.position}"
...
class NavalWargamingLexer:
    ...
    def match_line(self, line):
        ...
        while self.current_position < len(line):
            ...
            match = False
            for lexem_regex in NWG_LEXEM_REGEXES:
    ...
    def append_lexem(self, tag, data):
        ...
        """
        lexem = NavalWargamingLexem(tag, data, [self.current_line_number, self.current_position])
